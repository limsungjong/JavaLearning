...
/*
    ...
    1번 키부터 차례대로 할당된 문자들이 순서대로 담긴 문자열배열 keymap과 입력하려는
    문자열들이 담긴 문자열 배열 targets가 주어질 때,
    각 문자열을 작성하기 위해 키를 최소 몇 번씩 눌러야 하는지 순서대로 배열에 담아 return 하는 solution 함수를 완성해 주세요.

    단, 목표 문자열을 작성할 수 없을 때는 -1을 저장합니다.

    case1
    keymap = ["ABACD", "BCEFD"]	targets = ["ABCD","AABB"] result = [9, 4]

    case2
    keymap = ["AA"]	targets = ["B"]	result = [-1]

    case3
    keymap = ["AGZ", "BSSS"] targets = ["ASA","BGZ"] result = [4, 6]
 */


public class RoughKeyboard {
    public static void main(String[] args) {
        String[] case1x = {"ABACD", "BCEFD" };
        String[] case1y = {"ABCD", "AABB" };

        String[] case2x = {"AA" };
        String[] case2y = {"B" };

        String[] case3x = {"AGZ", "BSSS" };
        String[] case3y = {"ASA", "BGZ" };

        System.out.println(Arrays.toString(solution(case1x, case1y)));
        System.out.println(Arrays.toString(solution(case2x, case2y)));
        System.out.println(Arrays.toString(solution(case3x, case3y)));
    }
